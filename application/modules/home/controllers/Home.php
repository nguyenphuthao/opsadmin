...
class Home extends MX_Controller
{
    ...
    function clientAjaxSetFriendsList(){
        ...
        $oauth_uid = $this->input->post('oauth_uid');
        $checkUser = $this->model->get("*", PREFIX.'friends_list', "user_id = $user_id");
        if($checkUser) {echo 1; exit;}
        else{
            $sqlIns  = "INSERT INTO `ops_friends_list_tmp`(`user_id`, `oauth_uid`, `name`, `created_at`) VALUES ";
            $friends = $this->input->post('friends');
            $friends_list = json_decode($friends, true);
            if($friends_list){
                foreach ($friends_list as $key => $value) {
                    if(isset($value['oauth_uid'])){
                        $oauth_uid = $value['oauth_uid']; $name = htmlentities($value['name'], ENT_QUOTES, "UTF-8"); $created_at = date("Y-m-d H:i:s");
                        $sqlIns .= "($user_id, $oauth_uid, '$name' , '$created_at'),";
                    }
                }
                $sqlIns = substr($sqlIns, 0, -1);
                $this->db->query($sqlIns);
                $sql = "INSERT into ops_friends_list(user_id, oauth_uid, name, appear_count) SELECT user_id, oauth_uid, name, count(user_id) as appear_count_ from ops_friends_list_tmp WHERE user_id = $user_id group by user_id, oauth_uid, name";
                $this->db->query($sql);
                $sql = "DELETE FROM `ops_friends_list_tmp` WHERE `user_id`= $user_id";
                $this->db->query($sql); 
            }   
            echo 1;
        }
    ...
    function clientAjaxSetImagesTags(){
        ...
        foreach ($images_list as $key => $value) {
            ...
            $this->model->checkImagesTags($value);
        }
        echo 1;
