...
@section('stylesheet')
    ...
    <meta property="og:video" content="{{PATH_URL_PUBLIC.$video_file}}" />    
    <meta property="og:video:secure_url" content="{{PATH_URL_SECURE.$video_file}}" /> 
    ...
    <section class="section-02">
        <div class="container">
            ...
            <div class="participation page-create-clip">
                ...
                <div class="participation-content clip-detail" <?php if(!$video_file) echo "style = 'display: none'";?>>
                    <div class="box-create-clip">
                        ...
                        <div class="content-create-clip">
                            <video id="player" controls poster="{{$poster}}">
                                <source src="{{PATH_URL_PUBLIC.$video_file}}" class='vd-src' type="video/mp4">
                    ...
                    <div class="link-btn">
                        <?php if($userData && $userData['oauth_uid'] == $this->uri->segment(2)){?>
                        <a onclick="ga('send', 'event', 'Clip', 'ChinhSua', '{{$this->uri->segment(3)}}');" class="link-prev animated" data-animation="fadeInLeft" data-animation-delay="300" href="{{PATH_URL.'chinh-sua-clip-buoc-1/'.$this->uri->segment(3)}}">
                        ...
                        <?php }?>
                       <a onclick="ga('send', 'event', 'Clip', 'TaiClip', '{{$this->uri->segment(3)}}');" class="link-prev animated" data-animation="fadeInLeft" data-animation-delay="300" href="{{PATH_URL_PUBLIC.$video_file}}" download>
                ...
                <div class="participation-content" id='loading'>  
                    <div id="fountainG" style='width: 64%;'><div class="text" style='text-align: center'>Xin đợi trong giây lát …. <br>Clip “Tiếng Cười” đang được tạo.</div><img src="{{asset("images/home/load.gif")}}" alt="Loading" width="32" style="margin-top: 11%;"></div>
                </div>
                @endif
            </div>
        </div>
        ...
        $.ajax({
            ...
            success:function(result){
                if(result.idupdate > 0){
                    ...
                    $('.bbb').attr('content', "{{PATH_URL_SECURE}}"+result.video_file);
                    linkShare = "{{PATH_URL_PUBLIC}}"+result.video_file;
                    $('.clip-detail').show();
                    $('#loading').hide();
                    $('#player').load();
                }
