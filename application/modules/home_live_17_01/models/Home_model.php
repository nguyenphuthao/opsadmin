...
class Home_model extends MY_Model {
    ...
    public function checkFriendsList($data = array()){
    	...
    	if(!$user_id) return;
    	$result  = $this->model->get('id', PREFIX.'friends_list', "user_id = '$user_id' AND oauth_uid = '$oauth_uid'");
    	if(!$result){
            $data['appear_count'] = 1;
            $data['created_at']   = date("Y-m-d H:i:s");
    		$insert = $this->db->insert(PREFIX . 'friends_list', $data);
    	}else{
            $user = $this->getUser($user_id); 
    		$data['update_at'] = date("Y-m-d H:i:s");
            if($user->login_count == 0) $this->db->set('appear_count', 'appear_count+1', FALSE);
    		$this->db->update(PREFIX . 'friends_list', $data, array('id'=>$result->id));
    	}
    	return;
    }
